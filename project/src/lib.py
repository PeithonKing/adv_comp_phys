...
def label(dataPath):
    """
    Creates labels for the given halo
    Returns a numpy array with the following"""
    t504d = TNGData(dataPath)
    hid = 1

    data = np.zeros((5696477, 8))
    filled_till = 0
    
    for hid in [62,68,74,75,81,86,88,97,108,110,114,115,116,130,146,155,160,162,167,170,174,180,187]:
        shids_hid = t504d.shids_in_hid(hid)
        sh_lens = t504d.numpart_sh_in_h(hid)
        ...
        # print(halopartinfo.shape)
        # quit()
        
        data[filled_till:filled_till+halopartinfo.shape[0]] = halopartinfo
        filled_till += halopartinfo.shape[0]

        np.save("./new_test/"+str(hid)+".npy", halopartinfo)
        print("Done haloId:",hid)
        hid+=1
    print(data.shape)
    print(filled_till)
